...
def image2tensor(image):
    ...
    image -= np.array([103.939, 116.779, 123.68])  # center with respect to imagenet means
...
def extract_detections(frame, min_confidence=0.6, labels=None):
    ...
    try:
        results = np.hstack([results[:, 0].reshape(-1, 1), results[:, 2:]])
