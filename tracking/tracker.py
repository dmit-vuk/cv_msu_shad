...
class Tracker:
    ...
    def prev_detections(self):
        ...
        for i in range(min(self.lookup_tail_size, len(self.detection_history))):
            detection = self.detection_history[-1-i]
    ...
    def bind_tracklet(self, detections):
        ...
        for i in range(detections.shape[0]):
            for prev_detection in prev_detections:
                iou = iou_score(detections[i, 1:], prev_detection[1:])
                matches.append([iou, detections[i], prev_detection, i])
        ...
        for match in matches:
            if match[0] < threshold:
                break
            if match[-1] not in detect:
                detect.add(match[-1])
                detections[match[-1], 0] = match[2][0]
        # Step 4: assign new tracklet id to unmatched detections
        new_id = self.new_label()
        for i in range(detections.shape[0]):
            if i not in detect:
                detections[i, 0] = new_id
        return detection_cast(detections)
