...
class ISTrainer:
    def __init__(
        ...
        self.optim = torch.optim.AdamW(self.net.parameters(), lr=1e-4)
        self.bce_weight = 0.7
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            loss_bce = self.instance_loss(output["instances"], batch_data["instances"])
            loss_bce = torch.mean(loss_bce)

            pred = torch.sigmoid(output["instances"])
            dice = dice_loss(pred, batch_data["instances"])
            loss = loss_bce * self.bce_weight + dice * (1 - self.bce_weight)
...
def train_segmentation():
    ...
    #model.restore_from_checkpoint('../last_checkpoint.pth', "cuda:5")
